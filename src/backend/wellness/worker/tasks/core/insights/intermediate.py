...
def generate_intermediate_insights(
    ...
    insights = []
    insights.extend(_analyze_trends(metrics))

    if not any(
        t.key in (Insight.MOOD_TREND_UP, Insight.MOOD_TREND_DOWN) for t in insights
    ):
        insights.extend(_check_stability(metrics))

    return insights


def _analyze_trends(metrics: np.ndarray) -> list[response_dto.ActionItemResponseDTO]:
    n = metrics.shape[0]
    x = np.arange(n)
    ...
    for col, key_up, key_down, prio_up, prio_down in trend_config:
        ...
        threshold = (
            (0.15 if n < 10 else 0.1) * normalized_scale / n
            if normalized_scale > 0
            else 0.1
        )

        if p_value < (0.15 if n < 10 else 0.1) and relative_slope > 0.15:
            if slope > threshold:
                insights.append(
...
def _check_stability(metrics: np.ndarray) -> list[response_dto.ActionItemResponseDTO]:
    insights = []
    stability_config = [
    ...
    for col, key in stability_config:
        ...
        if cv < 0.1:
            insights.append(
                response_dto.ActionItemResponseDTO(
                    key, {"mean": round(mean_val, 1)}, Priority.LOW
                )
            )
    return insights
